(function (root, factory) {
      ...
      watch: {
        ...
        selectedPath: function (newPath, oldPath) {
          function makeTimespan(key, value) {
            ...
            views.sort(
              (v1, v2) =>
                viewOrder.indexOf(v1.camera) - viewOrder.indexOf(v2.camera)
            );
          ...
          if (newPath) {
            handlers.getFiles(newPath, (files) => {
              this.timespans = files.map(([key, value]) =>
                makeTimespan(key, value)
              );
            });
          } else {
            this.timespans = [];
        ...
        duration: function (duration) {
          this.controls.timespan =
            this.timespans.length > 0 ? this.timespans[0] : null;
          this.controls.playing = false;
      ...
      methods: {
        ...
        setDate: function (newDate) {
          if (newDate) {
            ...
            this.selectedPath = this.selectedTime
              ? this.selectedTime.time.relative
              : null;
          } else {
            this.times = [];
        ...
        deleteFiles: function (timespan) {
          ...
          if (
            confirm(
              `Are you sure you want to delete ${files.length} files from ${timespan.title}?`
            )
          ) {
        ...
        timespanTime: function (timespan) {
          ...
          time.setSeconds(
            time.getSeconds() - timespan.duration + Number(timespan.currentTime)
          );
      ...
      watch: {
        "controls.playing": function (playing, oldPlaying) {
          for (var timespan of this.timespans) {
            timespan.ended = false;
            timespan.playing =
              timespan == this.controls.timespan ? playing : false;
          }
        },
      },
      methods: {
  ...
  function createVideoComponent(handlers) {
    return Vue.component("SynchronizedVideo", {
      ...
      template: `<video ref="video" class="video" :class="view.camera" :src="view.file" :autoplay="timespan.playing" :playbackRate.prop="playbackRate" crossorigin="anonymous" @durationchange="durationChanged" @timeupdate="timeChanged" @ended="ended" title="Open in file explorer" @click="openExternal" @canplaythrough="loaded" playsinline></video>`,
      watch: {
        "timespan.playing": {
          handler: function (playing, oldPlaying) {
            ...
            if (video) {
              if (playing) {
                ...
                var currentTime =
                  this.timespan.currentTime -
                  (this.timespan.duration - video.duration);
        ...
        "timespan.currentTime": {
          handler: function (currentTime, oldTime) {
            ...
            if (video && !this.timespan.playing) {
              var adjustedTime =
                currentTime - (this.timespan.duration - video.duration);
      ...
      methods: {
        durationChanged: function (event) {
          ...
          this.timespan.duration = Math.max(
