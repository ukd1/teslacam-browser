...
function initialize() {
  ...
  ipcMain.on(
    "reopenFolders",
    (event) => (event.returnValue = services.reopenFolders())
  );
  ipcMain.on(
    "openFolder",
    (event, folder) =>
      (event.returnValue = setFolder(services.openFolder(folder)))
  );
  ipcMain.on(
    "getFiles",
    (event, p) =>
      (event.returnValue = services.getFiles(p, (f) =>
        path.join(services.args().folder, f)
      ))
  );
  ipcMain.on("openBrowser", (event) => browse());
  ipcMain.on("deleteFiles", (event, files) => services.deleteFiles(files));
  ipcMain.on("copyFilePaths", (event, filePaths) =>
    clipboard.writeText(services.copyFilePaths(filePaths))
  );
  ipcMain.on("deleteFolder", (event, folder) => services.deleteFolder(folder));
  ipcMain.on("copyPath", (event, p) =>
    clipboard.writeText(services.copyPath(p))
  );
