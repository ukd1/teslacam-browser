(function (root, factory) {
  if (typeof define === "function" && define.amd)
    define(["./helpers", "fs", "path", "express", "serve-index"], factory);
  ...
  function (helpers, fs, path, express, serveIndex) {
    ...
    function copyFilePaths(filePaths) {
      return filePaths
        .map((f) => `"${path.join(lastArgs.folder, f)}"`)
        .join(" ");
    }
    ...
    function openFolder(folder) {
      ...
      var dateGroups = helpers.groupBy(folderInfos, (g) =>
        g.date.toDateString()
      );
      ...
      var folderPathParts = folderNames.map((f, i) => {
        return { path: path.join(...folderNames.slice(0, i + 1)), name: f };
      });
      ...
      try {
        subfolders = fs
          .readdirSync(folder)
          .map((f) => {
            return { path: path.join(folder, f), name: f };
          })
    ...
    function initializeExpress(port) {
      ...
      expressApp.get("/openFolders", (request, response) =>
        response.send(openFolders())
      );
      expressApp.get("/reopenFolders", (request, response) =>
        response.send(reopenFolders())
      );
      expressApp.get("/args", (request, response) => response.send(args()));
      expressApp.get("/openDefaultFolder", (request, response) =>
        response.send(serveVideos(openFolder()))
      );
      expressApp.use("/openFolder", (request, response) =>
        response.send(serveVideos(openFolder(decodeURIComponent(request.path))))
      );
      expressApp.use("/copyFilePaths", (request, response) =>
        response.send(copyFilePaths(decodeURIComponent(request.path)))
      );
      expressApp.use("/copyPath", (request, response) =>
        response.send(copyPath(decodeURIComponent(request.path)))
      );
      expressApp.use("/files", (request, response) =>
        response.send(
          getFiles(decodeURIComponent(request.path), (p) => "videos/" + p)
        )
      );
